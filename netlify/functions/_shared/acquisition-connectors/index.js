...
const CONNECTORS = Object.freeze([laCountyEcaps, caCalEProcureCscr, openGovPublicBrowser, agentPublicSourceDiscovery]);
...
export function resolveConnector({ publisher, assignment }) {
  ...
  const explicitKey = txt(searchParameters.connector_key || searchParameters.connection_config?.connector_key || configuration.connector_key).toUpperCase();
  if (explicitKey) {
  ...
  const matched = CONNECTORS.find(item => item.publisherNames.some(name => name.toLowerCase() === publisherName) || item.hostnames.includes(hostname));
  if (!matched) throw new Error(`No tested publisher-specific connector is configured for ${publisher?.publisher_name || 'this publisher'}.`);
  return matched;
}
