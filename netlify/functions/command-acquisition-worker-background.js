...
exports.handler = async (event) => {
  ...
  try {
    ...
    const publishers = await db(`publisher_registry?state_code=eq.${stateCode}&verified=eq.true&select=id,publisher_name,state_code,search_endpoint,procurement_website,official_website`);
    ...
    if (!selected.length) {
      await db(`command_runs?id=eq.${commandRunId}`, {
        method: 'PATCH',
        body: JSON.stringify({ status: 'failed', aadp_state: 'FAILED', current_stage: 'NO_READY_ASSIGNMENTS', action_required: true, progress_value: 100, completed_at: now(), last_activity_at: now(), result_summary: `No READY verified publisher assignments are available for ${stateCode}.` })
      });
    ...
    for (let index = 0; index < selected.length; index++) {
      ...
      const created = await db('acquisition_runs', {
        method: 'POST',
        body: JSON.stringify({ command_run_id: commandRunId, assignment_id: assignment.id, status: 'RUNNING', started_at: now(), evidence: { runtime: 'NETLIFY_NATIVE', endpoint, publisher_name: assignment.publisher_name } })
      });
      ...
      try {
        ...
        failures++;
        await db(`acquisition_runs?id=eq.${acquisitionRun.id}`, { method: 'PATCH', body: JSON.stringify({ status: 'FAILED', retrieval_failures: 1, completed_at: now(), evidence: { runtime: 'NETLIFY_NATIVE', error: error instanceof Error ? error.message : String(error) } }) });
      }
