...
} from './_shared/publisher-discovery-taxonomy.js';
...
async function failRun(commandRunId, discoveryRunId, message) {
  ...
  if (discoveryRunId) await db(`publisher_discovery_runs?id=eq.${discoveryRunId}`, {
    method: 'PATCH',
    body: JSON.stringify({ status: 'FAILED', current_stage: 'ORCHESTRATION_FAILED', completed_at: timestamp, updated_at: timestamp, evidence: { error: message, runtime: 'NETLIFY_NATIVE', taxonomy_version: PUBLISHER_DISCOVERY_TAXONOMY_VERSION } })
  }).catch(() => null);
...
async function upsertAssignment(publisher, candidate, endpoint) {
  ...
  const values = {
    ...
    acquisition_method: txt(candidate.acquisition_method).toUpperCase(),
    search_endpoint: endpoint,
    search_parameters: { state_code: publisher.state_code, source: 'PUBLISHER_DISCOVERY', ...classification },
    authorized_status_range: ['OPEN', 'POSTED', 'ACTIVE'],
    pagination_instructions: {}, attachment_instructions: {}, amendment_instructions: {}, expected_source_identifiers: [],
...
export const handler = async event => {
  ...
  try {
    await patchRun(commandRunId, { status: 'running', aadp_state: 'RUNNING', current_stage: 'PUBLISHER_DISCOVERY', progress_value: 10, result_summary: 'Expanded official-source publisher research in progress.' });

    const discoveryRun = (await db('publisher_discovery_runs', { method: 'POST', body: JSON.stringify({
      command_run_id: commandRunId, state_code: stateCode,
      mission_name: `${stateCode} — Expanded Publisher Discovery and Acquisition Orchestration`,
      discovery_scope: discoveryScope,
      organization_types: [...PUBLISHER_DISCOVERY_ENTITY_CLASSES],
      intelligence_provider: 'OPENAI_WEB_SEARCH', operator_name: 'Executive Command Center',
      governance: { objective_validation_auto_admission: true, isolate_incomplete_candidates: true, preserve_contract_filters: true, taxonomy_version: PUBLISHER_DISCOVERY_TAXONOMY_VERSION },
      status: 'RUNNING', current_stage: 'PUBLISHER_DISCOVERY', started_at: now(),
      evidence: { source: 'EXECUTIVE_COMMAND_CENTER', runtime: 'NETLIFY_NATIVE', taxonomy_version: PUBLISHER_DISCOVERY_TAXONOMY_VERSION }
    }) }))?.[0];
    ...
    const prompt = buildPublisherDiscoveryPrompt({ stateCode, discoveryScope });
    const providerResponse = await fetch('https://api.openai.com/v1/responses', {
      method: 'POST', headers: { Authorization: `Bearer ${apiKey}`, 'Content-Type': 'application/json' },
      body: JSON.stringify({ model, reasoning: { effort: 'low' }, tools: [{ type: 'web_search', search_context_size: 'medium' }], input: prompt }),
    ...
    for (const candidate of candidates) {
      ...
      const candidateRow = (await db('publisher_discovery_candidates', { method: 'POST', body: JSON.stringify({
        ...
        vendor_registration_url: txt(candidate.vendor_registration_url) || null,
        procurement_platform: txt(candidate.procurement_platform) || null, technology_vendor: txt(candidate.technology_vendor) || null,
        registration_required: typeof candidate.registration_required === 'boolean' ? candidate.registration_required : null,
        ...
        review_notes: eligible
          ? `Objective validation passed. ${classification.publisher_role}; ${classification.opportunity_channel}; taxonomy ${classification.taxonomy_version}.`
          : `Missing official verification, supported acquisition method, or usable endpoint. ${classification.publisher_role}; ${classification.opportunity_channel}.`,
        reviewed_at: now()
      }) }))?.[0];
      staged++;
    ...
    await db(`publisher_discovery_runs?id=eq.${discoveryRunId}`, { method: 'PATCH', body: JSON.stringify({
      ...
      evidence: { runtime: 'NETLIFY_NATIVE', taxonomy_version: PUBLISHER_DISCOVERY_TAXONOMY_VERSION, entity_classes: PUBLISHER_DISCOVERY_ENTITY_CLASSES.length, candidates_staged: staged, assignments_ready: ready, exceptions_isolated: exceptions, duplicate_registry_matches: duplicates, autonomous_continuation: ready > 0 }
    }) });

    if (!ready) {
      await patchRun(commandRunId, { status: 'interrupted', aadp_state: 'PAUSED', current_stage: 'NO_READY_ASSIGNMENTS', progress_value: 60, records_acquired: staged, records_accepted: 0, records_rejected: exceptions, action_required: true, completed_at: now(), result_summary: `${staged} expanded publisher candidates staged, but none satisfied autonomous admission rules.` });
      return response(200, { ok: true, command_run_id: commandRunId, discovery_run_id: discoveryRunId, candidates_staged: staged, assignments_ready: 0, exceptions_isolated: exceptions, action_required: true, taxonomy_version: PUBLISHER_DISCOVERY_TAXONOMY_VERSION });
    }

    await patchRun(commandRunId, { status: 'running', aadp_state: 'RUNNING', current_stage: 'ACQUISITION_QUEUED', progress_value: 65, records_acquired: staged, records_accepted: ready, records_rejected: exceptions, action_required: false, result_summary: `${ready} expanded publisher assignments are READY. Acquisition Discovery is launching automatically.`, execution_evidence: { runtime: 'NETLIFY_NATIVE', discovery_run_id: discoveryRunId, taxonomy_version: PUBLISHER_DISCOVERY_TAXONOMY_VERSION, candidates_staged: staged, assignments_ready: ready, exceptions_isolated: exceptions, duplicate_registry_matches: duplicates, orchestration_mode: 'AUTONOMOUS' } });
    const dispatchStatus = await dispatchAcquisition(event, commandRunId, stateCode);
    return response(202, { ok: true, command_run_id: commandRunId, discovery_run_id: discoveryRunId, candidates_staged: staged, assignments_ready: ready, exceptions_isolated: exceptions, acquisition_dispatch_status: dispatchStatus, autonomous_continuation: true, taxonomy_version: PUBLISHER_DISCOVERY_TAXONOMY_VERSION });
  } catch (error) {
