...
export const handler = async event => {
  ...
  try {
    ...
    const eligiblePackageStatuses = new Set([
      '',
      'PACKAGE_NOT_STARTED',
      'PACKAGE_DISCOVERED',
      'PACKAGE_DOWNLOADING',
    ...
    const latestCaleprocureRun = (Array.isArray(packageRuns) ? packageRuns : []).find(run =>
      /cal\s*eprocure/i.test(String(run.mission_name || '')) ||
      /cal\s*eprocure/i.test(String(run.assigned_agent || ''))
    ) || null;

    const runStatus = lower(latestCaleprocureRun?.status);
    const automationHealth = runStatus === 'running' || runStatus === 'queued'
      ? 'RUNNING'
      : runStatus === 'completed'
        ? 'OPERATIONAL'
    ...
    return response(200, {
      ...
      caleprocure: {
        ...
        automation: {
          ...
          mode: 'CONTINUOUS_BATCH_CHAIN',
          normal_batch_size: 5,
          terminal_batch_size: '1-5',
          serialized: true
        },
        last_batch: latestCaleprocureRun ? {
          id: latestCaleprocureRun.id,
          status: latestCaleprocureRun.status,
          aadp_state: latestCaleprocureRun.aadp_state,
          current_stage: latestCaleprocureRun.current_stage,
          records_discovered: Number(latestCaleprocureRun.records_discovered || 0),
          records_acquired: Number(latestCaleprocureRun.records_acquired || 0),
          records_rejected: Number(latestCaleprocureRun.records_rejected || 0),
          failure_count: Number(latestCaleprocureRun.failure_count || 0),
          result_summary: latestCaleprocureRun.result_summary || null,
          started_at: latestCaleprocureRun.started_at || null,
          completed_at: latestCaleprocureRun.completed_at || null,
      ...
      sources: {
        ...
        caleprocure: 'state_contract_opportunities + command_runs'
      }
    });
  } catch (error) {
