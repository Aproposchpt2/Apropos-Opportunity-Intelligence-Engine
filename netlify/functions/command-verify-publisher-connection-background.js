...
export const handler = async event => {
  ...
  try {
    ...
    await db('connector_acceptance_registry?on_conflict=publisher_id,connector_key,connector_version', {
      ...
      body: JSON.stringify({ publisher_id: publisher.id, connector_key: connector.key, connector_version: connector.version || '1.0.0', acceptance_status: acceptanceStatus, validation_status: report.ready_for_acquisition ? 'PASSED' : 'WARNING', reconciliation_status: report.pagination_status === 'PASS' || report.pagination_status === 'TOLERATED_VARIANCE' ? 'MATCHED' : 'PARTIAL', publisher_reported_total: report.publisher_reported_total, records_acquired: report.records_parsed, acceptance_evidence: report, tested_at: now(), accepted_at: report.ready_for_acquisition ? now() : null, updated_at: now() })
    });
