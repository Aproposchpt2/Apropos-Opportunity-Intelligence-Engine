...
Deno.serve(async (request: Request) => {
  ...
  try {
    ...
    if (resumeRunId) {
      ...
      await db(`command_runs?id=eq.${resumeRunId}`, { method: 'PATCH', body: JSON.stringify({ aadp_state: 'RUNNING', status: 'running', current_stage: resumeStage, resume_source_stage: resumeStage, resumed_at: new Date().toISOString(), completed_at: null, execution_evidence: { ...(run.execution_evidence ?? {}), architecture: 'AADP-OS-V1.3-DURABLE', resume_requested_at: new Date().toISOString() } }) });
      await recordEvent(resumeRunId, null, 'AADP_RUN_RESUMED', 'AADP publisher run resumed through durable dispatcher', { resume_source_stage: resumeStage });
    } else {
      ...
      if (existing.length) {
        ...
        return json({ run_id: existing[0].id, status: existing[0].aadp_state, idempotent_replay: true, dispatch_scheduled: true }, 202);
      }
      ...
      const created = await db('command_runs', { method: 'POST', body: JSON.stringify({ idempotency_key: idempotencyKey, definition_id: definitions?.[0]?.id ?? null, publisher_assignment_id: assignmentId, status: 'queued', aadp_state: 'QUEUED', current_stage: 'PUBLISHER_ASSIGNMENT_CREATE', execution_evidence: { assignment_id: assignmentId, architecture: 'AADP-OS-V1.3-DURABLE', asynchronous_submission: true } }) });
      run = created[0];
      const tasks = await createTaskGraph(run.id as string);
      await recordEvent(run.id as string, null, 'AADP_RUN_SUBMITTED', 'AADP publisher acquisition run submitted to durable dispatcher', { assignment_id: assignmentId, task_count: tasks.length });
    }
    EdgeRuntime.waitUntil(invoke('aadp-run-dispatcher', { run_id: run.id }));
    return json({ run_id: run.id, status: 'QUEUED', asynchronous: true, durable_dispatch: true, poll: `command_runs?id=eq.${run.id}` }, 202);
  } catch (error) {
