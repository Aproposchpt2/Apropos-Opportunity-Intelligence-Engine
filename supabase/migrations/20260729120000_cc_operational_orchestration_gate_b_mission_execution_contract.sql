...
create trigger command_missions_touch_updated_at
before update on public.command_missions
...
begin
  if not public.command_is_operator() then
    raise exception 'Command Center operator authorization required';
  end if;
  select * into v_type from public.command_mission_types
   where mission_type_key=upper(btrim(p_mission_type_key)) and enabled=true;
  ...
  if v_type.state_context_required and (v_state is null or v_state !~ '^[A-Z]{2}$') then
    raise exception 'A two-letter state_code is required for this mission type';
  end if;

  if v_type.default_command_definition_id is null then
    v_authorization_state := 'BLOCKED';
    v_blocking_reasons := jsonb_build_array(jsonb_build_object(
      'code','RUNTIME_NOT_IMPLEMENTED',
      'message','Mission type is registered but no authorized runtime command definition exists.'
    ));
  else
    v_authorization_state := 'PENDING_AUTHORIZATION';
  end if;

  insert into public.command_missions(
    mission_type_key,mission_name,state_code,assigned_agent,mission_config,
    authorization_state,created_by,blocking_reasons,readiness_evidence
  ) values (
    v_type.mission_type_key,btrim(p_mission_name),v_state,btrim(p_assigned_agent),coalesce(p_mission_config,'{}'::jsonb),
    v_authorization_state,auth.uid(),v_blocking_reasons,
    jsonb_build_object(
      'mission_type_enabled',true,
      'state_context_satisfied',(not v_type.state_context_required or v_state is not null),
      'agent_assigned',true,
      'runtime_definition_available',(v_type.default_command_definition_id is not null)
    )
  ...
  values ('COMMAND_MISSION',v_mission_id::text,'MISSION_CREATED','OPERATOR',auth.uid()::text,
          'Generalized mission record created without self-authorization.',
          jsonb_build_object('authorization_state',v_authorization_state),
          jsonb_build_object('mission_type_key',v_type.mission_type_key,'state_code',v_state));
  return v_mission_id;
end $$;
...
begin
  ...
  select m.*,mt.default_command_definition_id into v_mission,v_definition_id
    from public.command_missions m
    join public.command_mission_types mt on mt.mission_type_key=m.mission_type_key
   where m.id=p_mission_id for update of m;
  ...
  update public.command_missions
     set authorization_state='AUTHORIZED',authorized_by=auth.uid(),authorized_at=now()
  ...
  values ('COMMAND_MISSION',p_mission_id::text,'MISSION_AUTHORIZED','OPERATOR',auth.uid()::text,
          'Operator explicitly authorized mission execution.',
          jsonb_build_object('authorization_state',v_mission.authorization_state),
          jsonb_build_object('authorization_state','AUTHORIZED'),
          jsonb_build_object('default_command_definition_id',v_definition_id));
  return p_mission_id;
end $$;
...
begin
  ...
  select default_command_definition_id into v_definition_id from public.command_mission_types where mission_type_key=v_mission.mission_type_key;
  ...
  update public.command_runs set
    mission_type_key=v_mission.mission_type_key,
    mission_name=v_mission.mission_name,
    state_code=v_mission.state_code,
    assigned_agent=v_mission.assigned_agent,
  ...
  values ('COMMAND_MISSION',p_mission_id::text,'MISSION_RUN_BOUND','SYSTEM',p_command_run_id,
          'Authorized mission bound to its mission-specific command run without changing canonical execution state.',
          jsonb_build_object('command_run_id',p_command_run_id),
          jsonb_build_object('mission_type_key',v_mission.mission_type_key,'definition_id',v_definition_id));
...
create or replace view public.command_mission_execution_contract
...
select
  ...
  m.state_code,
  m.assigned_agent,
  m.authorization_state,
  m.authorization_required,
  ...
  m.command_run_id,
  r.status canonical_execution_state,
  r.aadp_state mission_specific_state,
  case
    when m.authorization_state='CANCELLED' then 'CANCELLED'
  ...
  end executive_display_state,
  case
    when m.authorization_state in ('BLOCKED','PENDING_AUTHORIZATION') then true
    when r.action_required then true
    when r.status in ('interrupted','failed','completed_with_failures') then true
    when r.aadp_state='ESCALATED' then true
    else false
  end action_required,
  m.blocking_reasons,
  m.readiness_evidence,
  r.current_stage,
  r.progress_mode,
  r.progress_value,
  r.started_at,
  coalesce(r.last_activity_at,r.updated_at,m.updated_at) last_activity_at,
  r.completed_at,
  r.result_summary,
  r.registry_impact,
  r.report_reference,
...
create or replace view public.command_unified_stage_projection
with (security_invoker=true) as
select
  g.command_run_id,
  g.stage_key,
  ...
  g.failure_count,
  g.retry_count,
  coalesce(g.source_projection,'GENERIC') source_projection,
  g.evidence,
  g.updated_at
...
select
  ...
  a.display_name,
  a.display_state,
  row_number() over(partition by a.command_run_id order by coalesce(a.started_at,a.updated_at),a.stage_key)::integer sequence_number,
  null::numeric progress_value,
  a.started_at,
  a.completed_at,
  a.records_processed,
  a.warning_count,
...
create or replace view public.command_mission_notification_eligibility
with (security_invoker=true) as
select
  c.mission_id,
  c.command_run_id,
  c.state_code,
  case
    when c.executive_display_state='PENDING AUTHORIZATION' then 'ACTION_REQUIRED'
    ...
    when c.action_required then 'ACTION_REQUIRED'
    else null
  end notification_type,
  c.executive_display_state,
  c.action_required
...
-- Gate B intentionally does not create a new execution-state authority.
-- command_runs.status remains canonical; authorization state is governance state only.
-- Registered mission types without a command definition remain representable but non-executable.
